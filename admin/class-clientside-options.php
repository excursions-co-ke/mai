...
/*
 * Admin Options class
 * Contains default option values, deals with retrieving and saving Admin options
 */
...
class Clientside_Options
{
	...
	static function get_options_sections($section_slug = '')
	{
		...
		$options_sections = array(
			...
			// Admin Tools (enabling)
			'clientside-section-tools' => array(
				'slug' => 'clientside-section-tools',
				'title' => _x('Admin Tools', 'Option section name', 'clientside'),
				'page' => 'clientside-options-general',
				'options' => array(
	...
	// Return (array) the properties of one or more Admin plugin options
	static function get_option_info($option_slug = '')
	{
		...
		$options['admin-dashboard-title'] = array_merge(
			...
			array(
				...
				'default' => _x('Dashboard', 'Toolbar button text', 'clientside')
			)
		);
		...
		$options['enable-clientside'] = array_merge(
			...
			array(
				...
				'type' => 'checkbox',
				'default' => 1,
				'help' => _x('Disabling this option will completely disable Admin functionality for the specific user role. Administrators still have access to these option pages and Admin Tools to manage those for the other user roles.', 'Option description', 'clientside'),
				'role-based' => true
			)
		);
		...
		$options['enable-admin-theme'] = array_merge(
			...
			array(
				...
				'title' => _x('Enable Admin admin theming', 'Option title', 'clientside'),
				'secondary-title' => __('Enable for %s', 'clientside'),
				'type' => 'checkbox',
				'default' => 1,
				'help' => _x('Disabling this option will keep all Admin functionality besides the visual theming aspect for the specific user role.', 'Option description', 'clientside'),
				'role-based' => true
			)
		);
		...
		$options['enable-metabox-close-button'] = array_merge(
			...
			array(
				...
				'role-based' => true,
				'default' => 1,
				'help' => _x('When enabled, Admin adds a close-button to the corner of admin widgets/metaboxes.', 'Option description', 'clientside')
			)
		);
		...
		$options['enable-menu-counters'] = array_merge(
			...
			array(
				...
				'type' => 'checkbox',
				'default' => 1,
				'help' => _x('Admin adds additional item counts next to menu items. You can disable them with this option.', 'Option description', 'clientside')
			)
		);
		...
		$options['enable-notification-center'] = array_merge(
			...
			array(
				...
				'secondary-title' => __('Enable', 'clientside'),
				'help' => _x('The Admin Notification Center puts notifications away into a toolbar item instead of showing them directly on the page. Note that notifications still output normally on mobile.', 'Option description', 'clientside'),
				'type' => 'checkbox',
				'default' => 1
			)
		);
		...
		$options['disable-cli-error-handling'] = array_merge(
			...
			array(
				...
				'secondary-title' => __('Disable', 'clientside'),
				'help' => _x('Admin prevents PHP from printing errors before the document HTML and outputs them to the notification area instead. This feature can be disabled here if this causes problems with your own error logging solutions.', 'Option description', 'clientside'),
				'type' => 'checkbox',
				'default' => 0
			)
		);
		...
		$options['network-admins-only'] = array_merge(
			...
			array(
				...
				'secondary-title' => __('Make Admin only manageable by Network/Super Admins', 'clientside'),
				'help' => _x('Plugin Options and Admin Tools on all network sites will not be editable by site administrators.', 'Option description', 'clientside'),
				'type' => 'checkbox',
				'default' => 0,
				'network-option' => true
			)
		...
		$options['hide-plugin-entry'] = array_merge(
			...
			array(
				...
				'help' => _x('Hides Admin on individual network site\'s plugin listings for anyone except Network Admins, making Site Admins unable to deactivate the plugin. Note that if Admin is network-activated, it will already be hidden from the individual site\'s plugin list.', 'Option description', 'clientside'),
				'type' => 'checkbox',
				'default' => 0,
				'network-option' => true
			)
	...
	static function get_export()
	{
		...
		if (!in_array('custom-cssjs', $sections) && $strategy == 'exclude') {
			if (isset($return['custom-site-css'])) {
				unset($return['custom-site-css']);
			}
			if (isset($return['custom-site-js-header'])) {
				unset($return['custom-site-js-header']);
			}
			if (isset($return['custom-site-js-footer'])) {
				unset($return['custom-site-js-footer']);
			}
			if (isset($return['custom-admin-css'])) {
				unset($return['custom-admin-css']);
			}
			if (isset($return['custom-admin-js-header'])) {
				unset($return['custom-admin-js-header']);
			}
			if (isset($return['custom-admin-js-footer'])) {
	...
	// Process a Admin settings import request
...
?>
	...
	{
	?>
		<div class="clientside-form-image-preview <?php if (!$value) {
														echo '-empty';
													} ?>" id="<?php echo 'clientside-formfield-' . $field['name']; ?>-preview">
